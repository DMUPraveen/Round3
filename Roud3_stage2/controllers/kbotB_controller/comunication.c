...
{
    return (data_length + 3); //returning the length of the data plus head informtion in bytes
}
int construct_message(Command *scommand, char *byte_stream, int stream_length)
{
    if (get_byte_stream_length(scommand->data_length) == stream_length)
    {
        ...
        byte_stream[2] = scommand->caller_id; //setting the third byte to caller id
        for (int i = 0; i < scommand->data_length; i++)
        {
            byte_stream[i + 3] = (scommand->data)[i]; //storing the data in the byte stream
        }
        return 0; //success
        //printf("done");
    }
...
char get_message_caller_id(const char *message)
{
    return message[2];
}
...
{
    for (int i = 3; i < message_length; i++)
    {
        data_holder[i - 3] = message[i];
    }
}
...
int deconstruct_message(const char *message, int message_size, Command *scommand, char *data_holder, int data_holder_length)
{
    ...
    scommand->caller_id = get_message_caller_id(message);
