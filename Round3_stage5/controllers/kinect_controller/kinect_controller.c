...
int main(int argc, char **argv)
{
  ...
  */
  ...
  intilize_state_machine(&KbotB,1);
  ...
  while (wb_robot_step(TIME_STEP) != -1)
  {
    ...
    { 
      ...
      if(KbotB.next_state == GOING_TO_GRID){
        send_to_grid_position(KbotB.position,EMITTER,&KbotB);
        KbotB.state = GOING_TO_GRID;
      }
      if(KbotB.next_state == GOING_TO_TOWER){
        go_to_tower(EMITTER,&KbotB,find_tower_height(rfimage,rf_width));
        KbotB.state = GOING_TO_TOWER;
      }
    }
    ...
    if(KbotB.state == BOX_PICKED){
      request_robot_position(EMITTER, 1);
      KbotB.state = POSITION_REQUESTED;
      if(!are_all_slots_filled(&KbotB)){
      printf("all slots are filled\n");
      KbotB.next_state = GOING_TO_GRID;
      }
      else{
        KbotB.next_state = GOING_TO_TOWER;
      }
    }
    ...
    if(KbotB.state == TOWER_REACHED){
      place_box_on_tower_advanced(EMITTER,&KbotB,find_tower_height(rfimage,rf_width));
      KbotB.state = PLACING_BOX_ON_TOWER;
    }
    ...
    if (message_recieved)
    {
      ...
      {
        if(KbotB.state == POSITION_REQUESTED){
