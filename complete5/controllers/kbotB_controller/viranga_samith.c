...
int path_planing(double xr, double yr, double xb, double yb)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (((b1 - yr) - (yr - yb) * (a1 - xr) / (xr - xb)) * ((b3 - yr) - (yr - yb) * (a3 - xr) / (xr - xb)) > 0 && ((b2 - yr) - (yr - yb) * (a2 - xr) / (xr - xb)) * ((b4 - yr) - (yr - yb) * (a4 - xr) / (xr - xb)) > 0)
      {
        path = 0;
      }
      if ((yr < -0.3 && yb < -0.3) || (yr > 0.3 && yb > 0.3) || (xr > 1.2 && xb > 1.2) || (xr < -0.3 && xb < -0.3))
      {
        path = 0;
      }
      // if (yb-xb-1.5>0 &&
